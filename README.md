...
*   **User Profiles:**
    ...
    *   Profile displays:
        *   Username and Avatar (uploadable, with Vercel Avatars as fallback).
        *   "About Me" section.
...
*   **Admin Panel (`/admin`):**
    ...
    *   **Category Management:** Create new categories, view existing categories, edit category details (name, description), delete categories.
*   **Dynamic Updates:**
    *   Content (categories, topics, posts) updates dynamically after creation, editing, or deletion using Next.js Server Actions and `revalidatePath` for a smooth user experience without manual page refreshes.
...
2.  **Explore:**
    ...
    *   Reply to topics, try out the rich text editor features (Markdown, image upload, emoji).
...
4.  **User Profiles:** Click on a username to view their profile. If it's your own profile, you'll see an "Edit Profile" button.
...
│   │   ├── (auth)/     # Route group for auth pages (login, register) - this is now implicit
│   │   ├── login/
│   │   ├── register/
